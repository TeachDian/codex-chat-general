...
Portable users should keep `token-ledger.exe` and [WebView2Loader.dll](token-ledger-build/WebView2Loader.dll) in the same folder.
...
1. Download `token-ledger.exe`.
2. Download `WebView2Loader.dll`.
